 ...
 * 分享仅基于暂存列表选中项，不采集当前表格。
 */
(function (global) {
    'use strict';

    var stashSharePayload = null;
    var stashShareStashId = null;
    var _inited = false;
    ...
    function getStashListCount() {
        if (typeof global._tcStashListCount === 'number') return global._tcStashListCount;
        return 0;
    }

    function getSelectedStashId() {
        if (typeof global.getTcStashSelectedId === 'function') {
            return global.getTcStashSelectedId();
        }
        return '';
    }

    function resolveSharePayload() {
        return stashSharePayload;
    }

    function clearStashSharePayload() {
        stashSharePayload = null;
        stashShareStashId = null;
    }

    function closeCreateModal() {
        clearStashSharePayload();
        closeModal('tc-share-create-modal');
    }
    ...
    function mountShareModalsToBody() {
        ['tc-share-create-modal', 'tc-share-list-modal', 'tc-share-comments-modal'].forEach(function (id) {
            var el = $(id);
    ...
    function updateShareButtons() {
        ...
        var count = getStashListCount();
        var hasItems = count > 0;
        if (createBtn) {
            createBtn.disabled = !hasItems;
            if (!hasItems) {
                createBtn.title = '暂无暂存，无法分享';
            } else {
                var sel = getSelectedStashId();
                createBtn.title = sel ? '分享选中的暂存' : '请先单击选中一条暂存';
            }
        }
    }

    function resolveSharePayload(stashPayload) {
        var live = null;
        if (typeof global.tcCollectLiveSharePayload === 'function') {
            live = global.tcCollectLiveSharePayload();
        }
        if (live && hasTableData(live)) return live;
        if (!stashPayload) return stashPayload;
        try {
            var cloned = JSON.parse(JSON.stringify(stashPayload));
    ...
    function openCreateModal(opts) {
        opts = opts || {};
        if (!opts.payload) {
            alertBox('请先在暂存列表中单击选中一条数据，再点击分享评审。', { title: '请先选择暂存' });
            return;
        }
        ensureLoggedIn().then(function () {
            stashSharePayload = resolveSharePayload(opts.payload);
            stashShareStashId = opts.stash_id ? String(opts.stash_id) : null;
            var payload = resolveSharePayload();
            if (!hasTableData(payload)) {
                clearStashSharePayload();
                alertBox('所选暂存没有可分享的表格数据。', { title: '无法分享' });
                return;
            }
            ...
            if (expiresEl) expiresEl.value = String(opts.expires_days || 30);
            if (commentsEl) commentsEl.checked = opts.comment_enabled !== false;
            openModal('tc-share-create-modal');
        }).catch(function (err) {
    ...
    function submitCreate() {
        ...
        var expiresEl = $('tc-share-create-expires');
        var commentsEl = $('tc-share-create-comments');
        var payload = resolveSharePayload(stashSharePayload);
        if (!hasTableData(payload)) {
            alertBox('所选暂存没有可分享的表格数据。', { title: '无法分享' });
            return;
        }
        stashSharePayload = payload;
        var btn = $('tc-share-create-submit');
        if (btn) btn.disabled = true;
        fetchJson('/api/test-cases/shares', {
            method: 'POST',
            body: JSON.stringify({
                title: titleEl ? titleEl.value : '',
                stash_id: stashShareStashId || getSelectedStashId() || '',
    ...
    function fetchStashForShare(stashId) {
        if (typeof global.fetchTcStashById === 'function') {
            return global.fetchTcStashById(stashId);
        }
        return fetchJson('/api/test-case-stashes/' + encodeURIComponent(stashId)).then(function (data) {
            return data && data.stash;
        });
    }

    function openShareFromStash(stashId) {
        if (!stashId) return;
        ensureLoggedIn().then(function () {
            return fetchStashForShare(stashId);
        }).then(function (stash) {
            if (!stash || !stash.payload) throw new Error('暂存数据无效');
            openCreateModal({
                title: (stash.title || '') + ' 评审',
                payload: stash.payload,
                stash_id: stash.id || stashId
            });
        }).catch(function (err) {
            if (onAuthRequired(err)) return;
            alertBox(err.message || '加载暂存失败', { title: '无法分享' });
        });
    }

    function openShareFromSelection() {
        if (getStashListCount() === 0) return;
        var stashId = getSelectedStashId();
        if (!stashId) {
            alertBox('请先在暂存列表中单击选中一条数据。', { title: '请先选择暂存' });
            return;
        }
        openShareFromStash(stashId);
    }
    ...
    function bindShareDelegation() {
        ...
        document.addEventListener('click', function (ev) {
            ...
            if (target.closest('#tc-share-create-cancel')) {
                ev.preventDefault();
                closeCreateModal();
                return;
            }

            if (target.closest('#tc-share-create-submit')) {
                ev.preventDefault();
                submitCreate();
                return;
            }
            ...
            if (revokeBtn) {
                ...
                if (!rid || !global.confirm('确定作废该分享链接？作废后评审人将无法访问。')) return;
            ...
            if (ev.target && ev.target.id === 'tc-share-create-modal') {
                closeCreateModal();
                return;
            }
    ...
    global.TcShareWorkbench = {
        ...
        openShareFromStash: openShareFromStash,
