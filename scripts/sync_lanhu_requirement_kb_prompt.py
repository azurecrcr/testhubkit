#!/usr/bin/env python3
"""将 lanhu-requirement-kb/SKILL.md 同步到提示词库（JSON 种子 + MySQL upsert）。"""
...
from core.services.prompts.prompt_library_service import (  # noqa: E402
    LANHU_REQUIREMENT_KB_ENTRY_ID,
    get_lanhu_requirement_kb_seed_entry,
    upsert_prompt_entry,
)
...
def sync_builtin_json(entry: dict) -> None:
    ...
    for i, item in enumerate(data):
        if isinstance(item, dict) and item.get("id") == LANHU_REQUIREMENT_KB_ENTRY_ID:
            data[i] = slim
            found = True
            break
    if not found:
...
def main() -> None:
    ...
    entry = get_lanhu_requirement_kb_seed_entry()
    if not entry.get("body"):
        raise SystemExit("lanhu-requirement-kb/SKILL.md not found or empty")
