/**
 * 线程组 · 查看结果树 JMX 片段（隔离模块）
 */
(function (global) {
    'use strict';

    var Catalog = global.JmsTgViewResultsTreeCatalog;
    ...
    function genXml(cfg, indent, escapeXml) {
        ...
        var xml = indent + '<ResultCollector guiclass="ViewResultsFullVisualizer" testclass="ResultCollector" testname="' + name + '" enabled="' + enabled + '">\n';
        if (comments) {
    ...
    function parseFromJmxNode(node) {
        ...
        if (gui !== 'ViewResultsFullVisualizer') return null;
        return Catalog.normalizeConfig({
            name: node.getAttribute('testname') || '查看结果树',
    ...
    global.JmsTgViewResultsTreeJmx = {
        genXml: genXml,
