    ...
    function paintBatchBar(stats) {
        // "本次生成"横幅已永久隐藏，仅同步导图按钮可见性
        syncMindmapReviewBtnVisibility();
    }
    ...
    function syncRowHighlight() {
        if (isMindmapBatchActive()) {
            removeTableAdoptButtons();
            syncMindmapReviewBtnVisibility();
            return;
        }
        document.querySelectorAll('#table-body tr[data-row-index]').forEach(function (tr) {
            ...
            tr.classList.remove('tc-row-adopt-accept', 'tc-row-adopt-reject', 'tc-row-adopt-edit', 'tc-row-validate-issue');
            var fb = state.feedback[idx];
            if (fb === 'accept' || fb === 'export') tr.classList.add('tc-row-adopt-accept');
            else if (fb === 'edit_accept') tr.classList.add('tc-row-adopt-edit');
            else if (fb === 'reject') tr.classList.add('tc-row-adopt-reject');
        });
    ...
    function postFeedback(rowIndex, action) {
        syncBatchStateFromCore();
        if (!state.batchId || !isBatchRow(rowIndex)) return Promise.resolve();
        state.feedback[rowIndex] = action;
        if (global.tcGenBatchCore && global.tcGenBatchCore.state) {
            global.tcGenBatchCore.state.feedback[rowIndex] = action;
            if (typeof global.tcGenBatchCore.paintProgress === 'function') global.tcGenBatchCore.paintProgress();
            if (typeof global.tcGenBatchCore.syncRowHighlight === 'function') global.tcGenBatchCore.syncRowHighlight();
        }
        syncRowHighlight();
        updateLocalBatchStats();
        if (isLocalBatchId()) return Promise.resolve();
        return fetchJson('/api/test-cases/generation-batches/' + state.batchId + '/feedback', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ row_index: rowIndex, action: action })
        }).then(function (stats) {
            state.feedback = stats.feedback || state.feedback;
            paintBatchBar(stats);
            syncRowHighlight();
        }).catch(function () { /* 离线仍保留本地状态 */ });
    }
    ...
    function acceptAllBatch() {
        if (isLocalBatchId()) {
            ...
            updateLocalBatchStats();
            syncRowHighlight();
            if (global.tcGenBatchCore) {
                global.tcGenBatchCore.state.feedback = state.feedback;
                if (typeof global.tcGenBatchCore.paintProgress === 'function') global.tcGenBatchCore.paintProgress();
                if (typeof global.tcGenBatchCore.syncRowHighlight === 'function') global.tcGenBatchCore.syncRowHighlight();
            }
            toast('已全部采纳', { variant: 'success', duration: 2400 });
            return Promise.resolve();
        }
        if (!state.batchId) return Promise.resolve();
        return fetchJson('/api/test-cases/generation-batches/' + state.batchId + '/accept-all', { method: 'POST' })
            .then(function (s) {
    ...
    function rejectAllBatch() {
        if (isLocalBatchId()) {
            ...
            toast('已全部拒绝', { variant: 'info', duration: 2400 });
            return Promise.resolve();
        }
        if (!state.batchId) return Promise.resolve();
        return fetchJson('/api/test-cases/generation-batches/' + state.batchId + '/reject-all', { method: 'POST' })
            .then(function (s) {
                state.feedback = s.feedback || {};
                paintBatchBar(s);
                syncRowHighlight();
    ...
    function postRowFeedback(arg) {
        if (!arg || arg.rowIndex == null) return;
        postFeedback(parseInt(arg.rowIndex, 10), arg.action);
    }
    ...
    function onGenerationSuccess(meta) {
        ...
        if (state.batchId && String(state.batchId).indexOf('local-') !== 0 && state.loggedIn) {
            fetchJson('/api/test-cases/generation-batches/' + state.batchId + '/close', { method: 'POST' }).catch(function () {});
        }
        if (typeof global.tcShowGenBatchBar === 'function') {
            global.tcShowGenBatchBar(rowCount, batchMode);
        } else {
            showBatchBar(true);
            paintBatchBar({ row_count: rowCount, processed: 0, adoption_rate: 0, mode: batchMode });
        }
        ...
        if (global.tcGenBatchCore && global.tcGenBatchCore.state) {
            ...
            global.tcGenBatchCore.state.feedback = {};
        }
        ...
        if (shouldUseLocalStatsStorage()) {
            syncLocalBatchToStorage();
        } else {
            fetchJson('/api/test-cases/generation-batches', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    mode: batchMode,
                    ai_mode: state._batchAiMode,
                    rag_used: state._batchRagUsed,
                    row_count: rowCount
                })
            }).then(function (batch) {
                if (batch && batch.id) {
                    var prevBatchId = state.batchId;
                    state.batchId = batch.id;
                    if (global.tcGenBatchCore && global.tcGenBatchCore.state) {
                        global.tcGenBatchCore.state.batchId = batch.id;
                    }
                    migrateBatchValidationCacheKey(VALIDATE_SCOPE_SINGLE, prevBatchId, batch.id);
                    snapshotBatchForValidateScope(VALIDATE_SCOPE_SINGLE);
                    vScopeData(VALIDATE_SCOPE_SINGLE).validationBatchSnapshot = cloneBatchSnapshot(vScopeData(VALIDATE_SCOPE_SINGLE).batchSnapshot);
                }
            }).catch(function () { /* 保留 local 批次 */ });
        }

        window.setTimeout(function () {
            if (tableLinked) {
                injectRowAdoptButtons();
                syncRowHighlight();
            ...
            updateLocalBatchStats();
            if (global.tcGenBatchCore) {
                if (tableLinked && typeof global.tcGenBatchCore.injectRowAdoptButtons === 'function') {
                    global.tcGenBatchCore.injectRowAdoptButtons();
                } else if (typeof global.tcGenBatchCore.syncRowHighlight === 'function') {
                    global.tcGenBatchCore.syncRowHighlight();
                }
            }
    ...
    function injectRowAdoptButtons() {
        if (isMindmapBatchActive() || state._batchTableLinked === false) {
            removeTableAdoptButtons();
            syncMindmapReviewBtnVisibility();
            return;
        }
        if (!state.batchId || !state.batchRowCount) return;
        document.querySelectorAll('#table-body tr[data-row-index]').forEach(function (tr) {
            var idx = parseInt(tr.getAttribute('data-row-index'), 10);
            if (!isBatchRow(idx)) return;
            if (tr.querySelector('.tc-adopt-actions')) return;
            var inner = tr.querySelector('.tc-td-actions .tc-row-actions-inner');
            if (!inner) return;
            var wrap = document.createElement('div');
            wrap.className = 'tc-adopt-actions';
            wrap.innerHTML =
                '<button type="button" class="tc-adopt-btn tc-adopt-btn--yes" data-idx="' + idx + '" title="采纳此行">✓</button>' +
                '<button type="button" class="tc-adopt-btn tc-adopt-btn--no" data-idx="' + idx + '" title="拒绝此行">✕</button>';
            inner.appendChild(wrap);
        });
    }

    function bindAdoptDelegation() {
        var tbody = $('table-body');
        if (!tbody || tbody._tcAdoptBound) return;
        tbody._tcAdoptBound = true;
        tbody.addEventListener('click', function (e) {
            var yes = e.target.closest('.tc-adopt-btn--yes');
            var no = e.target.closest('.tc-adopt-btn--no');
            if (yes) { e.stopPropagation(); postFeedback(parseInt(yes.getAttribute('data-idx'), 10), 'accept'); }
            if (no) { e.stopPropagation(); postFeedback(parseInt(no.getAttribute('data-idx'), 10), 'reject'); }
        });
    }

    function wrapDeleteRow() {
        if (typeof global.deleteRow !== 'function' || global.deleteRow._tcWrapped) return;
        var orig = global.deleteRow;
        global.deleteRow = function (index) {
            if (state.batchId && index >= 0 && index < state.batchRowCount) {
                postFeedback(index, 'reject');
            }
            return orig.apply(this, arguments);
        };
        global.deleteRow._tcWrapped = true;
    }

    function wrapSaveEditRow() {
        if (typeof global.saveEditRow !== 'function' || global.saveEditRow._tcWrapped) return;
        var orig = global.saveEditRow;
        global.saveEditRow = function () {
            var idx = global.editingRowIndex;
            var r = orig.apply(this, arguments);
            if (state.batchId && idx != null && idx >= 0) postFeedback(idx, 'edit_accept');
            return r;
        };
        global.saveEditRow._tcWrapped = true;
    }

    function wrapRenderTableBody() {
        if (typeof global.renderTableBody !== 'function' || global.renderTableBody._tcWrapped) return;
        var orig = global.renderTableBody;
        global.renderTableBody = function () {
            orig.apply(this, arguments);
            injectRowAdoptButtons();
            syncRowHighlight();
        };
        global.renderTableBody._tcWrapped = true;
    }
    ...
    function shouldRunSingleAutoValidateAfterGeneration(meta) {
        ...
        if (meta.autoValidate !== true) return false;
    ...
    function appendValidateLlmReasoning(scope, chunk) {
        ...
        if (vData.validateProgress) {
            if (typeof patchValidationReasoningDom === 'function' &&
    ...
    function setValidateLlmReasoning(scope, text) {
        scope = normalizeValidateScope(scope);
        vScopeData(scope).validateLlmReasoning = String(text || '');
        if (vScopeData(scope).validateProgress) {
            if (typeof patchValidationReasoningDom === 'function' &&
                patchValidationReasoningDom(scope)) {
                /* reasoning patched */
            } else {
    ...
    function fetchRequirementTextFromDbForCurrentPage() {
        ...
        return fetch('/api/test-cases/lanhu-page-cache', {
            ...
            if (hit && String(hit.content_text || '').trim()) {
                return String(hit.content_text).trim();
            }
            return '';
    ...
    function runAutoValidationAfterGeneration() {
        if (!isAutoValidateFormatEnabled()) return Promise.resolve();
        if (isSingleGenAutoValidateBlocked()) return Promise.resolve();
        return scheduleAutoValidationAfterTablePaint();
    }
