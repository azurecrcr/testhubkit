...
/** 导图容器已展示且有尺寸时才允许初始化 SMM（避免 Scrollbar rbox 在隐藏容器上报错） */
function tcMindmapStashEditorContainerReady() {
...
function tcMindmapStashPeekMindSnapshot() {
    var live = null;
    if (window.TcSmmEditor && typeof TcSmmEditor.isInstanceReady === 'function' && TcSmmEditor.isInstanceReady()) {
        if (typeof TcSmmEditor.captureMindSnapshot === 'function') {
            try { live = TcSmmEditor.captureMindSnapshot(); } catch (eLive) { /* ignore */ }
        }
    }
    ...
    if (ext && ext.data) {
        return tcMindmapStashCloneMind(ext) || ext;
    }
    if (typeof tcMindmapCaptureMindSnapshot === 'function') {
        try { return tcMindmapCaptureMindSnapshot(); } catch (eCap) { /* ignore */ }
    }
    return null;
}
...
function tcMindmapStashPrepareCapture() {
    ...
    if (tcMindmapStashEditorContainerReady() && window.TcSmmEditor && typeof TcSmmEditor.ensure === 'function') {
...
function collectTcMindmapStashPayload(opts) {
    ...
    var sourceRows = (tcMindmapCasesData && tcMindmapCasesData.length)
        ? tcMindmapCasesData
        : (opts.allowTableRowFallback === true ? testCasesData : []);
    ...
    return {
        ...
        columns: cols,
        rows: rows,
        provenance: tcMindmapProvenanceArrayForStashPayload(rows.length),
...
function getTcMindmapStashSaveBlockReason() {
    var mind = tcMindmapStashPeekMindSnapshot();
...
function tcMindmapStashHasContent() {
    var colCount = (typeof tableColumns !== 'undefined' && tableColumns) ? tableColumns.length : 0;
    if (typeof tcTableRowsHaveCellContent === 'function') {
        if (tcTableRowsHaveCellContent(tcMindmapCasesData, colCount)) return true;
        if (tcTableRowsHaveCellContent(testCasesData, colCount)) return true;
    } else if (tcMindmapCasesData && tcMindmapCasesData.length) {
        return true;
    }
    var ext = typeof tcMindmapExternalMindData !== 'undefined' ? tcMindmapExternalMindData : null;
    if ((!ext || !ext.data) && typeof tcMindmapCommittedExternalMind !== 'undefined') {
    ...
    if (ext && ext.data && tcMindmapStashMindHasUserNodes(ext)) return true;
    return false;
}
...
function applyTcMindmapStashDocument(doc, opts) {
    opts = opts || {};
    ...
    tcMindmapCasesProvenance = tcProvenanceArrayFromStashPayload(p, rows.length);
    tcEnsureMindmapProvenanceLength();
    if (opts.syncTableStore === true) {
        ...
        markedRows = new Set();
        selectedRows.clear();
    } else if (window.TcWorkbenchData &&
        typeof window.TcWorkbenchData.shouldSyncTableStoreOnMindmapStashRestore === 'function' &&
        window.TcWorkbenchData.shouldSyncTableStoreOnMindmapStashRestore()) {
        testCasesData = rows.map(function (r) { return r.slice(); });
        testCasesProvenance = tcProvenanceArrayFromStashPayload(p, rows.length);
        tcEnsureProvenanceLength();
        markedRows = new Set();
...
function applyTcMindmapStashRestoreFlow(doc) {
    ...
    if (!tcMindmapStashHasContent()) {
        applyTcMindmapStashDocument(doc, { silentToast: true });
        tcAppToast('恢复成功', { variant: 'success', duration: 3400 });
        return;
    }
    if (typeof closeTcStashPreviewModal === 'function') closeTcStashPreviewModal();
...
function syncTcMindmapStashChrome() {
    ...
    if (listHint) {
        listHint.textContent = '离开页面时会自动写入「自动保存-」快照';
    }
