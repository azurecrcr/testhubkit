...
function runAiTableToolbar(scope, loadingBtn, options) {
    ...
    if (window.TcRequirementCaseStore && typeof window.TcRequirementCaseStore.captureGenerationContext === 'function') {
        window.TcRequirementCaseStore.captureGenerationContext();
    }
    ...
    function finishBtn() {
        ...
        if (typeof syncTcPromptSendBtnState === 'function') syncTcPromptSendBtnState();
        if (!genCancelled && window.TcRequirementCaseStore && typeof window.TcRequirementCaseStore.persistAfterGeneration === 'function') {
            window.TcRequirementCaseStore.persistAfterGeneration();
        }
        if (options.suppressLeftPanelUi) {
        ...
        tcPrepareVisualPayload(requestData).then(function (payload) {
            ...
            return fetch('/api/test-case-generator', fetchOpts)
                ...
                .catch(function(error) {
                    if (!isGenerationRunCurrent(generationRunToken) || (error && error.name === 'AbortError')) return;
                    tcAppAlert(error.message || String(error), { variant: 'error', title: '请求未完成', hint: '请检查网络或稍后在网络稳定时重试。' });
                });
        }).finally(finishBtn);
    }
    ...
    resolveAiGenerateMergeMode().then(function(choice) {
        ...
        tcSyncListTableBeforeAiGenerate().then(function() {
