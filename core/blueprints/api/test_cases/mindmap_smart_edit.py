...
from core.services.test_cases.mindmap_smart_edit_service import (
    iter_mindmap_smart_edit_stream_events,
    run_mindmap_smart_edit,
)
...
def _resolve_mindmap_smart_edit_request(data: dict) -> tuple[dict[str, object] | None, tuple[dict, int] | None]:
    user_prompt = str(data.get("user_prompt") or "").strip()
    mindmap_snapshot = data.get("mindmap_snapshot")
    use_builtin = bool(data.get("use_builtin"))

    if not user_prompt:
        return None, ({"success": False, "error": "请输入编辑指令"}, 400)
    if not isinstance(mindmap_snapshot, dict):
        return None, ({"success": False, "error": "缺少思维导图用例快照"}, 400)
    if not list(mindmap_snapshot.get("columns") or []):
        return None, ({"success": False, "error": "导图列定义为空，请先应用模板"}, 400)
    ...
    user_message_history = data.get("user_message_history")
    if user_message_history is None:
        user_message_history = data.get("conversation_history")
    ...
    return {
        ...
        "mindmap_snapshot": mindmap_snapshot,
...
def register_routes(bp: Blueprint) -> None:
    @bp.route("/test-cases/mindmap-smart-edit", methods=["POST"])
    def mindmap_smart_edit_cases():
        try:
            data = request.json or {}
            resolved, err = _resolve_mindmap_smart_edit_request(data)
            if err:
                body, code = err
                return jsonify(body), code

            result = run_mindmap_smart_edit(
                base_url=str(resolved["base_url"]),
                api_key=str(resolved["api_key"]),
                model=str(resolved["model"]),
                user_prompt=str(resolved["user_prompt"]),
                mindmap_snapshot=resolved["mindmap_snapshot"],
                temperature=resolved["temperature"],
                visual_context_id=resolved.get("visual_context_id"),
                user_id=resolved.get("user_id"),
                user_message_history=resolved.get("user_message_history"),
            )
    ...
    @bp.route("/test-cases/mindmap-smart-edit/stream", methods=["POST"])
    def mindmap_smart_edit_cases_stream():
        try:
            data = request.json or {}
            resolved, err = _resolve_mindmap_smart_edit_request(data)
            if err:
            ...
            def generate():
                ...
                for event in iter_mindmap_smart_edit_stream_events(
                    base_url=str(resolved["base_url"]),
                    api_key=str(resolved["api_key"]),
                    model=str(resolved["model"]),
                    user_prompt=str(resolved["user_prompt"]),
